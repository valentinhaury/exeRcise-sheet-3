...
pixar_films %>% 
  filter(count > 1) %>%
  ggplot(aes(y = metacritic, x = fct_reorder2(film, series, -as.numeric(release_date)))) +
    geom_bar(stat = "identity") +
    facet_wrap(~series, scales = "free_x") +
    labs(x="Movie", y = "Metacritic-Score", title = "Metacritic-Score for Pixar-movies grouped by Series") +
    theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
TODO -> Serien Auf der x-achse und filme darüber als Gruppen an separaten Balken
